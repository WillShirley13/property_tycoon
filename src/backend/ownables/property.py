...
class Property(Ownable):
    ...
    def set_current_sell_value(self, new_value: int = None, downgrade: bool = False, upgrade: bool = False) -> None:
        if downgrade:
    ...
    def downgrade_property(self, bank) -> None:
        try:
            ...
            if self.hotel == 1:
                hotel_cost = PROPERTY_BUILD_COSTS[self.property_group.get_value()]["hotel"]
            ...
            # Otherwise, remove a house if we have any
                ...
                house_cost = PROPERTY_BUILD_COSTS[self.property_group.get_value()]["house"]
