...
class Property(Ownable):
    def __init__(
        ...
        self.houses: int = 0
        self.hotel: int = 0
        self.next_upgrade_cost: int = PROPERTY_BUILD_COSTS[self.property_group.value][
            "house"
        ]
    ...
    def set_owner_owns_all_properties_in_group(
        self, owner_owns_all_properties_in_group: bool
    ) -> None:
        self.owner_owns_all_properties_in_group = owner_owns_all_properties_in_group

    def set_current_sell_value(
        self, new_value: int = None, downgrade: bool = False, upgrade: bool = False
    ) -> None:
        if downgrade:
    ...
    def upgrade_property(self, bank) -> None:
        ...
        try:
            ...
            if self.get_houses() == 4 and self.get_is_eligible_upgrade():
                ...
                self.set_hotel(1)
                self.set_value(hotel_cost, increase=True)
            ...
            # Otherwise, add a house if eligible
                ...
                self.set_value(house_cost, increase=True)
    ...
    def downgrade_property(self, bank) -> None:
        ...
        try:
            ...
            if self.hotel == 1:
                ...
                self.set_hotel(0)
                self.set_houses(4)
                self.set_value(hotel_cost, increase=False)
            ...
            # Otherwise, remove a house if we have any
                ...
                self.set_houses(self.get_houses() - 1)
                self.set_value(house_cost, increase=False)
                self.set_current_sell_value(downgrade=True)
                self.check_max_difference_between_houses_owned_is_1_within_property_group(
                    self.get_houses() - 1
                )
    ...
    def set_rent_cost(self) -> None:
        if not self.owner_owns_all_properties_in_group:
            ...
            self.owner_owns_all_properties_in_group
            and self.houses == 0
            and self.hotel == 0
        ):
    ...
    def check_max_difference_between_houses_owned_is_1_within_property_group(
        self, this_props_houses_after_change: int
    ) -> bool:
