...
from ..constants import (ONE_UTILITY_RENT_DICE_MULTIPLIER,
                         TWO_UTILITY_RENT_DICE_MULTIPLIER)
...
class Utility(Ownable):
    ...
    def get_rent_due_from_player(self, player: "Player") -> int:
        ...
        if self.num_of_utilities_owned_by_owner == 1:
            rent = sum(player.get_recent_dice_rolls()) * \
                ONE_UTILITY_RENT_DICE_MULTIPLIER
    ...
    def set_owner_owns_all_utilities(
            self, owner_owns_all_utilities: bool) -> None:
    ...
    def set_num_of_utilities_owned_by_owner(
            self, num_of_utilities_owned_by_owner: int) -> None:
