...
class TimeLimitChoiceDisplay:
    def __init__(
        self, screen_width: int, screen_height: int, screen: pygame.Surface
    ) -> None:
        # Store screen
    ...
    def setup_buttons(self) -> None:
        ...
        for i, time in enumerate(self.time_options):
            ...
            button_y: int = (
                self.screen_height // 2 - 50 + (button_height + button_margin) * i
            )

            # Store button rect and associated time value
            self.buttons.append(
                (pygame.Rect(button_x, button_y, button_width, button_height), time)
            )
    ...
    def draw(self) -> None:
        ...
        for button, time in self.buttons:
            ...
            pygame.draw.rect(
                self.screen, (30, 100, 30), button, 2, border_radius=10
            )  # Add border
