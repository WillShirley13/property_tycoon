...
class TimeLimitChoiceDisplay:
    def __init__(self, screen_width: int, screen_height: int,
        ...
        self.title_size: int = max(
            int(screen_height * 0.05), 24)  # Minimum size of 24
        self.button_size: int = max(
            int(screen_height * 0.04), 20)  # Minimum size of 20

        # Initialize fonts with fallback to default font if Arial not
        # available
        try:
            ...
            self.button_font: pygame.font.Font = pygame.font.SysFont(
                "Arial", self.button_size)
        except BaseException:
            self.title_font: pygame.font.Font = pygame.font.SysFont(
                None, self.title_size, bold=True)
            self.button_font: pygame.font.Font = pygame.font.SysFont(
                None, self.button_size)

        # Create button layout
        # Will be set in setup_buttons
    ...
    def setup_buttons(self) -> None:
        ...
        for i, time in enumerate(self.time_options):
            # Center buttons horizontally, stack vertically with
            # margin
            button_x: int = (self.screen_width - button_width) // 2
            button_y: int = self.screen_height // 2 - \
                50 + (button_height + button_margin) * i
    ...
    def draw(self) -> None:
        ...
        for button, time in self.buttons:
            ...
            button_color: Tuple[int, int, int] = self.BUTTON_HOVER_COLOR if button.collidepoint(
            ...
            pygame.draw.rect(
                ...
                button,
                border_radius=10)
            pygame.draw.rect(self.screen, (30, 100, 30), button,
                             2, border_radius=10)  # Add border
