...
def render_text_for_space(screen, space, text, font_size, space_type, space_width, is_corner=False):
    ...
    if space_type == 'left':
        ...
        for line in name_lines:
            text_surface = bold_font.render(line, True, (0, 0, 0))
            # Instead of rotating 180 degrees, we'll use a different approach
            # First render the text normally, then flip it both horizontally and vertically
            # This preserves the correct reading order while making it appear upside down
            flipped_surface = pygame.transform.flip(text_surface, True, True)
            text_rect = flipped_surface.get_rect(center=(space.centerx, y_start))
            screen.blit(flipped_surface, text_rect)
            y_start += bold_font.size(line)[1] + 2
        
        # Render price at the top if it exists
        if property_price:
            price_surface = regular_font.render(property_price, True, (0, 0, 0))
            # Use the same flipping approach for the price
            flipped_price = pygame.transform.flip(price_surface, True, True)
            price_rect = flipped_price.get_rect(center=(space.centerx, space.top + padding + flipped_price.get_height()//2))
            screen.blit(flipped_price, price_rect)
