...
class StartScreenDisplay:
    def __init__(self, screen_width: int, screen_height: int,
        ...
        self.TEXT_COLOR: Tuple[int, int, int] = (
            0, 0, 0)  # Black for names
        self.BUTTON_COLOR: Tuple[int, int, int] = (
            148, 0, 0)  # Red for button
        ...
        self.INPUT_BORDER_COLOR: Tuple[int, int, int] = (
            200, 200, 200)
        ...
        try:
            ...
            self.title_font = pygame.font.SysFont(
                None, self.title_size, bold=True)
            self.subtitle_font = pygame.font.SysFont(
                None, self.subtitle_size)
            self.button_font = pygame.font.SysFont(
                None, self.button_size, bold=True)
    ...
    def setup_input_boxes(self) -> None:
        ...
        for i in range(5):
            ...
            box_y: int = self.screen_height // 2 - \
                50 + (box_height + box_margin) * i
    ...
    def setup_start_button(self) -> None:
        ...
        button_y: int = self.input_boxes[-1].bottom + \
            int(self.screen_height * 0.025) * 2
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.start_button.collidepoint(event.pos):
                        ...
                        player_names: List[str] = [
                            name.capitalize() for name in self.input_texts if name.strip()]

                        if len(player_names) != len(
                                set(player_names)):
                            # Show an error message about duplicate
                            # names
                ...
                if event.type == pygame.KEYDOWN:
                    if self.active_box != -1:
                        if event.key == pygame.K_RETURN:
                            ...
                            if len(
                                    self.input_texts[self.active_box]) < 20:
    ...
    def show_duplicate_error(self) -> None:
        ...
        error_surface: pygame.Surface = self.subtitle_font.render(
            error_message, True, (255, 0, 0))
        error_rect: pygame.Rect = error_surface.get_rect(
    ...
    def draw(self) -> None:
        ...
        background_scaled = pygame.transform.scale(
            self.BACKGROUND_COLOR, (self.screen_width, self.screen_height))
        ...
        subtitle_surface: pygame.Surface = self.subtitle_font.render(
            "Enter player names (1-5 players)", True, self.INPUT_BG_COLOR)
        ...
        for i, box in enumerate(self.input_boxes):
            ...
            label_surface: pygame.Surface = self.input_font.render(
                label_text, True, self.INPUT_BG_COLOR)
            ...
            if label_x < 10:  # Ensure label doesn't go off screen
                ...
                label_y = box.y + \
                    (box.height - label_surface.get_height()) // 2
        ...
        button_color: Tuple[int, int, int] = self.BUTTON_HOVER_COLOR if self.start_button.collidepoint(
        ...
        pygame.draw.rect(self.screen, (30, 100, 30),
                         self.start_button, 2, border_radius=10)  # Border
