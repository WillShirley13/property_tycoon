...
class StartScreenDisplay:
    def __init__(self, screen_width: int, screen_height: int, screen: pygame.Surface):
        ...
        # Colors
        self.BACKGROUND_COLOR = pygame.image.load("src/frontend/art_assets/background.png")
        self.TITLE_COLOR: Tuple[int, int, int] = (255, 255, 255)  # White for title
        self.TEXT_COLOR: Tuple[int, int, int] = (0, 0, 0)
        self.BUTTON_COLOR: Tuple[int, int, int] = (148, 0, 0)  # Red for button
        ...
        try:
            ...
            self.subtitle_font: pygame.font.Font = pygame.font.SysFont("Arial", self.subtitle_size)
            self.button_font: pygame.font.Font = pygame.font.SysFont("Arial", self.button_size, bold=True)
            self.input_font: pygame.font.Font = pygame.font.SysFont("Arial", self.input_size)
        except:
    ...
    def setup_start_button(self) -> None:
        ...
        button_y: int = self.input_boxes[-1].bottom + int(self.screen_height * 0.025) * 2
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.start_button.collidepoint(event.pos):
                        ...
                        player_names: List[str] = [name.capitalize() for name in self.input_texts if name.strip()]

                        if len(player_names) != len(set(player_names)):
                            # Show an error message about duplicate names
                            self.show_duplicate_error()
                        elif player_names:  # Ensure at least one player
                            return player_names
    ...
    # if you have more than one player with the same name
    def show_duplicate_error(self) -> None:
        """Display an error message if there are duplicate names."""
        error_message: str = "Error: Player names must be unique!"
        error_surface: pygame.Surface = self.subtitle_font.render(error_message, True, (255, 0, 0))
        error_rect: pygame.Rect = error_surface.get_rect(center=(self.screen_width // 2, self.screen_height // 1.1))
        self.screen.blit(error_surface, error_rect)

    def draw(self) -> None:
        """Draw the start screen elements."""
        # Fill background
        background_scaled = pygame.transform.scale(self.BACKGROUND_COLOR, (self.screen_width, self.screen_height))
        self.screen.blit(background_scaled, (0, 0))

        # Draw title
        title_surface: pygame.Surface = self.title_font.render("Property Tycoon", True, self.TITLE_COLOR)
        title_rect: pygame.Rect = title_surface.get_rect(center=(self.screen_width // 2, self.screen_height // 4))
        self.screen.blit(title_surface, title_rect)

        # Draw subtitle
        subtitle_surface: pygame.Surface = self.subtitle_font.render("Enter player names (1-5 players)", True, self.INPUT_BG_COLOR)
        subtitle_rect: pygame.Rect = subtitle_surface.get_rect(center=(self.screen_width // 2, title_rect.bottom + 30))
        self.screen.blit(subtitle_surface, subtitle_rect)

        # Draw input boxes and labels
        for i, box in enumerate(self.input_boxes):
            # Draw box
            border_color: Tuple[int, int, int] = (0, 150, 0) if i == self.active_box else self.INPUT_BORDER_COLOR
            ...
            label_surface: pygame.Surface = self.input_font.render(label_text, True, self.INPUT_BG_COLOR)
            ...
            # Draw input text
            text_surface: pygame.Surface = self.input_font.render(self.input_texts[i], True, self.TEXT_COLOR)
            self.screen.blit(
                text_surface,
        ...
        button_color: Tuple[int, int, int] = self.BUTTON_HOVER_COLOR if self.start_button.collidepoint(mouse_pos) else self.BUTTON_COLOR
        pygame.draw.rect(self.screen, button_color, self.start_button, border_radius=10)
        pygame.draw.rect(self.screen, (30, 100, 30), self.start_button, 2, border_radius=10)  # Border

        # Draw button text
        button_text: pygame.Surface = self.button_font.render("Start Game", True, (255, 255, 255))
        button_text_rect: pygame.Rect = button_text.get_rect(center=self.start_button.center)
