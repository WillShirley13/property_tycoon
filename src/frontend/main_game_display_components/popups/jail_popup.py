...
class JailPopup:
    ...
    def draw(self, player: Player) -> None:
        ...
        pay_color: Tuple[int, int, int] = self.BUTTON_COLOR if has_enough_money else self.BUTTON_DISABLED_COLOR
        self.pay_fine_hover = self.pay_fine_hover if has_enough_money else False
        self.draw_button(self.pay_fine_button, "Pay £50 Fine", self.pay_fine_hover, pay_color)
    ...
    def handle_events(
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left mouse button
            ...
            # Option 3: Pay fine (Option to sell assets if not enough funds)
            if self.pay_fine_button.collidepoint(mouse_pos):
                try:
                    # Try to pay the fine - will throw exception if not enough funds
                    jail.pay_fine_for_release(player, free_parking, go, bank)
                    ...
                    try:
                        ...
                        print(f"Player {player.get_name()} has been declared bankrupt!")
                        self.result = "bankrupt"
                        return True
    ...
    def show(self, player: Player, jail: Jail, free_parking: FreeParking, bank: Bank, go: Go) -> Optional[str]:
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if self.handle_events(event, player, jail, free_parking, bank, go):
                    running = False  # Exit the popup if a choice was made

                # Check again if player became bankrupt during event handling
                if player.get_is_bankrupt():
                    self.result = "bankrupt"
                    running = False
