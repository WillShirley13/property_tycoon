...
class GameCardPopup:
    # Initialize the popup for when a player lands on a game card
    # space
    def __init__(self, x: int, y: int, width: int,
        ...
        # Create button rectangle for the continue option (this is for
        # collision detection)
        self.continue_button: pygame.Rect = pygame.Rect(
            self.x + 20,  # 20px from left edge of popup
            self.y + self.button_y,
            # Position corresponds to the y-position used in draw()
            self.button_width,
            self.button_height,
        )
    ...
    def draw_button(self, surface: pygame.Surface,
                    button_rect: pygame.Rect, text: str, hover: bool) -> None:
    ...
    def show(
        ...
        # Try to process the card, allow player to sell assets if they don't
        # have enough money
        ...
        while not card_processed:
            try:
                ...
                if action is None:
                    print(
                        "Player closed the popup without selling anything")
                    try:
