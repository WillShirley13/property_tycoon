...
class AuctionPopup:
    def __init__(self, x: int, y: int, width: int, height: int, screen: pygame.Surface):
        ...
        self.popup_surface: pygame.Surface = pygame.Surface((width, self.height))
    ...
    def draw(self, property_obj: Property, players: List[Player], current_player: Player) -> None:
        ...
        for detail in details:
            detail_surface = self.text_font.render(detail, True, self.POPUP_TEXT_COLOR)
        ...
        # Initialize input fields for all players (except current player)
        ...
        for player in players:
            if player != current_player:
                ...
                player_surface = self.text_font.render(player_text, True, self.POPUP_TEXT_COLOR)
                player_rect = player_surface.get_rect(left=20, top=y_position)
                ...
                bg_color = (220, 255, 220) if self.input_active.get(player.get_name(), False) else self.INPUT_BG_COLOR
                pygame.draw.rect(self.popup_surface, bg_color, input_rect, 0, 5)

                # Draw border
                pygame.draw.rect(self.popup_surface, self.POPUP_BORDER_COLOR, input_rect, 1, 5)

                # Draw input text
                text_surface = self.text_font.render(self.input_texts[player.get_name()], True, self.POPUP_TEXT_COLOR)
                text_rect = text_surface.get_rect(left=input_rect.left + 5, top=input_rect.top + 5)
        ...
        self.submit_hover = self.submit_button.collidepoint(relative_mouse_pos)

        button_color = self.BUTTON_HOVER_COLOR if self.submit_hover else self.BUTTON_COLOR

        # Draw button with rounded corners
        pygame.draw.rect(self.popup_surface, button_color, self.submit_button, 0, 5)
        pygame.draw.rect(self.popup_surface, self.POPUP_BORDER_COLOR, self.submit_button, 1, 5)

        # Add text to the button
        text_surface = self.button_font.render("Submit Bids", True, self.POPUP_TEXT_COLOR)
        text_rect = text_surface.get_rect(center=self.submit_button.center)
    ...
    def handle_events(self, event: pygame.event.Event, players: List[Player]) -> bool:
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left click
            ...
            if self.submit_button.collidepoint(relative_mouse_pos):
                ...
                for player_name, bid_text in self.input_texts.items():
                    try:
                        # If bid is not an integer, players bid is void
                        bid = int(bid_text)
                        if bid > 0:
                            # set player bid
                            for player in players:
                ...
                if valid_bids:
                    # Find highest bidder
                    highest_bidder = max(valid_bids.items(), key=lambda x: x[1])  # compare each bid (value for player:bid dictionary)
                    self.auction_result = {
            ...
            for player_name, is_active in self.input_active.items():
                if is_active:
                    ...
                    # if character is numeric or empty string, add to input text
            ...
            for event in pygame.event.get():
                ...
                if self.handle_events(event, players):
                    if self.auction_result:
                        self.auction_result["winner"].purchase_property(property_obj, bank, self.auction_result["amount"])
                        print(f"{self.auction_result['winner'].get_name()} purchased {property_obj.get_name()} for £{self.auction_result['amount']}.")
                    running = False
