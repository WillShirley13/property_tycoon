...
class EndOfGamePopup:
    def __init__(self, x: int, y: int, width: int, height: int, screen: pygame.Surface, winner):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        ...
        # Set fonts
        try:
            self.title_font: pygame.font.Font = pygame.font.SysFont("Arial", 36, bold=True)
            self.subtitle_font: pygame.font.Font = pygame.font.SysFont("Arial", 28, bold=True)
            self.details_font: pygame.font.Font = pygame.font.SysFont("Arial", 20)
            self.button_font: pygame.font.Font = pygame.font.SysFont("Arial", 24, bold=True)
        except:
            self.title_font = pygame.font.SysFont(None, 36, bold=True)
            self.subtitle_font = pygame.font.SysFont(None, 28, bold=True)
            self.details_font = pygame.font.SysFont(None, 20)
            self.button_font = pygame.font.SysFont(None, 24, bold=True)
    ...
    def draw(self) -> None:
        ...
        # Draw border
        pygame.draw.rect(self.surface, self.BORDER_COLOR, (0, 0, self.width, self.height), 3, 10)
        ...
        winner_surface = self.subtitle_font.render(winner_text, True, self.TEXT_COLOR)
        winner_rect = winner_surface.get_rect(center=(self.width // 2, 100))
        ...
        details = [
            ...
            f"Properties Owned: {sum(len(properties) for properties in self.winner.get_owned_properties().values())}",
        ]

        for detail in details:
            detail_surface = self.details_font.render(detail, True, self.TEXT_COLOR)
            detail_rect = detail_surface.get_rect(center=(self.width // 2, y_pos))
        ...
        congrats_surface = self.subtitle_font.render(congrats_text, True, self.TITLE_COLOR)
        congrats_rect = congrats_surface.get_rect(center=(self.width // 2, y_pos + 20))
        self.surface.blit(congrats_surface, congrats_rect)

        # Check for button hover
        mouse_pos = pygame.mouse.get_pos()
        relative_mouse_pos = (mouse_pos[0] - self.x, mouse_pos[1] - self.y)
        self.button_hover = self.close_button.collidepoint(relative_mouse_pos)

        # Draw close button with hover effect
        button_color = self.BUTTON_HOVER_COLOR if self.button_hover else self.BUTTON_COLOR
        pygame.draw.rect(self.surface, button_color, self.close_button, 0, 10)
        pygame.draw.rect(self.surface, self.BORDER_COLOR, self.close_button, 2, 10)

        # Add button text
        button_text = "Exit Game"
        button_text_surface = self.button_font.render(button_text, True, self.BUTTON_TEXT_COLOR)
        button_text_rect = button_text_surface.get_rect(center=self.close_button.center)
