...
class SellOrMortgagePopup:
    def __init__(self, x: int, y: int, width: int, height: int, screen: pygame.Surface):
        ...
        self.sell_or_mortgage_choice: Optional[str] = None  # Will store the user's choice (sell or mortgage)
    ...
    def draw_select_property_popup(self, player: Player, sell: bool = False, mortgage: bool = False) -> None:
        ...
        self.draw_button(self.popup_surface, self.close_button_rect, "Close", False, (220, 100, 100))
    ...
    def draw_sell_or_mortgage_popup(self) -> None:
        ...
        details = [
            ...
            (f"Property Houses: {self.selected_property.get_houses()}" if isinstance(self.selected_property, Property) else "No houses"),
            (f"Property Hotel: {self.selected_property.get_hotel()}" if isinstance(self.selected_property, Property) else "No hotel"),
        ...
        # draw sell button
        self.sell_button_rect = pygame.Rect(self.width // 4, self.height // 2 - 55, self.width // 2, 50)
        self.draw_button(
            self.popup_surface,
    ...
    def sell_or_mortgage_choice_handle_events(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            sell_hit: bool = (
                adjusted_pos[0] >= 10 and adjusted_pos[0] <= 10 + (self.width - 20) and adjusted_pos[1] >= 70 and adjusted_pos[1] <= 70 + 50
            )

            # check if mortgage button was clicked
            mortgage_hit: bool = (
                adjusted_pos[0] >= 10 and adjusted_pos[0] <= 10 + (self.width - 20) and adjusted_pos[1] >= 130 and adjusted_pos[1] <= 130 + 50
            )
    ...
    def handle_selling_property_events(self, event: pygame.event.Event) -> Tuple[bool, bool]:
        if event.type == pygame.MOUSEBUTTONDOWN:
            # adjust mouse position relative to popup
            adjusted_pos: Tuple[int, int] = (
                event.pos[0] - self.x,
            ...
            if self.sell_button_rect.collidepoint(adjusted_pos):
                print(f"Selling property: {self.selected_property.get_name()} for {self.selected_property.get_current_sell_value()}")
                (
                    self.current_player.sell_property(self.selected_property, self.bank)
                    if self.sell_or_mortgage_choice == "sell"
                    else self.current_player.mortgage_property(self.selected_property, self.bank)
                )
    ...
    def show(self, player: Player, bank: Bank) -> Tuple[Optional[str], Optional[Property]]:
        ...
        while property_running:
            ...
            if self.selected_property and property_details_showing:
                ...
                self.draw_select_property_popup(player, sell=(action == "sell"), mortgage=(action == "mortgage"))
            ...
            for event in pygame.event.get():
                ...
                if property_details_showing:
                    ...
                    return_to_selection, close_popup = self.handle_selling_property_events(event)
        ...
        # Return both the action (sell/mortgage) and the last selected property that was sold/mortgaged
        return action if transaction_completed else None, last_property_sold
