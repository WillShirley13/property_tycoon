...
def raise_capital(
    ...
    is_selling_house_or_hotel: bool = False,
) -> None:
...
while True:
    ...
    # Section: Roll dice
    ###################################################
    move_result: list[list[int], int] | None = (
        current_player.move_player()
    )  # return list of dice rolls and new position or None if player rolled 3 doubles
    ...
    match space_on_board:
        case Ownable():
            ...
            if space_on_board.get_owner() == current_player:
                ...
                print(
                    f"{current_player.get_name()} landed on a property that is owned by the bank, {space_on_board.name}. Offer to purchase the property."
                )

                if not current_player.get_is_first_circuit_complete():
                    print(
                        f"{current_player.get_name()} is on the first circuit. They cannot make purchases until they have completed the first circuit."
                    )
                    players.pop(0)
                    continue
        ...
        case FreeParking():
            print(
                f"{current_player.get_name()} landed on Free Parking. All previous fines that have been collected are now being paid out to the player."
            )
        ...
        case GameCard():
            ...
            if game_space_helper[space_on_board] in pot_luck_card_positions:
                ...
                while not opportunity_knocks_card_loop_flag:
                    try:
                        opportunity_knocks.process_card(
                            player=current_player,
                            bank=bank,
                            card_id=card_info[1],
                            free_parking=free_parking,
        ...
        while not upgrade_loop_flag:
            ...
            if len(properties_eligible_for_upgrade) == 0:
                ...
                upgrade_choice = input(
                    "Would you like to upgrade any of your properties? (reply with the index of the property you want to upgrade or 'n' if you don't want to upgrade any properties): "
                )
