...
class TestJailMechanism(unittest.TestCase):
    ...
    def test_pay_fine_to_get_out_of_jail(self):  
        ...
        self.assertFalse(self.player.get_is_in_jail())  # Assert player is out of jail

        # Assert the fine was added to Free Parking
        self.assertEqual(self.free_parking.get_fines_collected(), 50)

        # Uncomment if using a mock for free_parking.add_fine
        # self.free_parking.add_fine.assert_called_with(self.jail.get_release_cost(), self.player)
